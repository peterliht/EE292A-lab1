...
int main(int argc, char **argv) {
	...
	for (unsigned i = 0; i < NUM_DIGITS; i++){
		...
		if (use_fixed_point)
			snprintf(weights_file, 256, "weights_fxp/weights_fxp_%d", i);
		else
...
void classify() {
	...
	input_weights_buffer = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(float) * FEATURE_COUNT * NUM_DIGITS, NULL, &status);
	checkError(status, "Error: could not create input image buffer");
	...
	status = clEnqueueWriteBuffer(queue, input_weights_buffer, CL_TRUE, 0, sizeof(float) * FEATURE_COUNT * NUM_DIGITS, input_weights, 0, NULL, NULL);
	checkError(status, "Error: could not copy data into device");
